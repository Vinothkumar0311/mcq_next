...
//     const studentId = user.userSIN;
//     try {
//       const data = await getStudentBookings(studentId);
//       console.log("Student Bookings:", data);
//     } catch (error) {
...
//       await bookSlotApi(studentId, slotId);
...
//                             <Users className="w-4 h-4 text-gray-500" />
//                             <span>
//                               Seats: {slot.seatsLeft}/{slot.maxSeats}
//                             </span>
//                           </div>
...
const StudentSlotBooking = () => {
  ...
  const [studentBookings, setStudentBookings] = useState<number[]>([]);
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  ...
  // Fetch bookings of this student
  // const loadStudentBookings = async () => {
  //   const studentId = user.userSIN;
  //   try {
  //     const response = await getStudentBookings(studentId);
  //     console.log("Student Bookings:", response);

  //     const bookedIds = response?.data?.map((b: any) => b.slotId);
  //     setStudentBookings(bookedIds || []);
  //   } catch (error) {
  //     console.error("Failed to fetch student bookings", error);
  //   }
  // };
  ...
  try {
    ...
    // backend returns: { success: true, data: [ { slotId, ... } ] }
    // const bookedSlotIds = response.data.map((b: any) => b.slotId);

    console.log("Raw Student Bookings Response:", response);

    const bookedSlotIds =
      response?.map((b: any) => Number(b.slotId)) || [];


    console.log("Booked Slot IDs:", bookedSlotIds);
// console.log("studentBookings State:", studentBookings);


    setStudentBookings(bookedSlotIds);
  } catch (error) {
    console.error("Failed to fetch student bookings", error);
  }
};
  ...
  const getBookingStatus = (slot: Slot) => {
    const isRegistered = studentBookings.includes(slot.id);
    ...
    if (diffMinutes > 15) {
      const bookingOpenTime = new Date(
        slotDateTime.getTime() - 15 * 60 * 1000
      );

      return {
        canBook: false,
        message: `Opens @ ${bookingOpenTime.toLocaleTimeString("en-IN", {
          hour12: false,
          hour: "2-digit",
          minute: "2-digit",
        })}`,
        ...
        <div className="bg-gradient-to-r from-purple-600 to-purple-700 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">Book Assessment Slot</h1>
                <p className="text-purple-100">Choose your preferred time slot</p>
              </div>
            </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2 text-blue-800">
                <p className="font-medium">📋 Booking Guidelines</p>
                <ul className="text-sm space-y-1">
                  <li>• Booking opens 15 min before start time</li>
                  <li>• You cannot book the same slot twice</li>
                  <li>• Be present on time</li>
