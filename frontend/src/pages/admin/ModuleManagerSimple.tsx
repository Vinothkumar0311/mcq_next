...
import { Badge } from "@/components/ui/badge";
import { useToast } from "@/hooks/use-toast";
import { ArrowLeft, Plus, BookOpen, Trash2 } from "lucide-react"; // Removed unused 'Edit'
import AdminLayout from "@/components/AdminLayout";
import { useNavigate, useParams } from "react-router-dom";
// --- API Imports (from new version) ---
import { getModulesByCourse, createModule, deleteModule } from "@/api/moduleApi";

const ModuleManagerSimple = () => {
  const { toast } = useToast();
  const navigate = useNavigate();
  const { courseId } = useParams();
  const [course, setCourse] = useState<any>(null); // Kept for header
  const [modules, setModules] = useState<any[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [formData, setFormData] = useState({
    title: '',
  ...
  // --- API Fetch Function (from new version) ---
  const fetchModules = async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Unable to fetch modules", variant: "destructive" });
    }
  };

  // --- Merged useEffect ---
  // Loads course details for the header (from old version)
  // Fetches modules from the API (from new version)
  useEffect(() => {
    // Load course data for header (same as old version)
    const courses = JSON.parse(localStorage.getItem('courses') || '[]');
    const foundCourse = courses.find((c: any) => c.id === parseInt(courseId!));
    setCourse(foundCourse || { id: courseId, name: 'Course' });

    // Load modules from API (from new version)
    if (courseId) {
      fetchModules();
    }
  }, [courseId]);

  // --- API handleSubmit (from new version, with validation from old) ---
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createModule(Number(courseId), formData);
      toast({ title: "Created", description: "Module created successfully" });
      setFormData({ title: '', description: '', duration: 60 });
      setShowForm(false);
      fetchModules(); // Refresh list from API
    } catch {
      toast({ title: "Error", description: "Failed to create module", variant: "destructive" });
    }
  };

  // --- API handleDelete (from new version) ---
  ...
  // --- UI (from old version, with minor badge update) ---
  return (
    <AdminLayout>
      <div className="p-6 space-y-6">
        ...
        {/* Create Module Button (from old version) */}
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-semibold">Course Modules</h2>
          <Button 
            onClick={() => setShowForm(!showForm)}
        ...
        {/* Modules List (from old version, with API data) */}
        <div className="space-y-4">
          {modules.map((module, index) => (
            <Card key={module.id} className="border hover:border-purple-300 transition-colors">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-sm">{module.description}</p>
                      <div className="flex items-center gap-2 mt-1">
                        {/* Updated to use 'module.status' from API */}
                        <Badge variant="secondary">{module.status || 'Draft'}</Badge>
                        <span className="text-sm text-gray-500">{module.duration} min</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {/* "Add Test" Button (from old version) */}
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => navigate(`/admin/modules/${module.id}/tests/create`)}
                    ...
                    {/* Delete Button (from old version, new handler) */}
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleDelete(module.id)}
