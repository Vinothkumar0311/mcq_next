...
// const defaultVenueData: Partial<Venue> = { name: "", block: "", maxSeats: 30 };
// const defaultSlotData: Partial<Slot> = {
//   date: new Date().toISOString().split("T")[0],
//   startTime: "09:00",
//   endTime: "10:00",
...
//   const [venues, setVenues] = useState<Venue[]>([]);
//   const [slots, setSlots] = useState<Slot[]>([]);
//   const [venueForm, setVenueForm] = useState(defaultVenueData);
//   const [slotForm, setSlotForm] = useState(defaultSlotData);
//   const [slotDate, setSlotDate] = useState<Date | undefined>(new Date());
//   const [editingVenue, setEditingVenue] = useState<Venue | null>(null);
//   const [editingSlot, setEditingSlot] = useState<Slot | null>(null);
//   const [loading, setLoading] = useState(false);
...
//     e.preventDefault();
//     if (!venueForm.name || !venueForm.block) return;
...
//     e.preventDefault();
//     if (!slotDate || !slotForm.startTime || !slotForm.endTime) return;
...
//               <Building className="h-4 w-4 mr-1" /> Venues
//             </TabsTrigger>
//             <TabsTrigger value="slot">
//               <Clock className="h-4 w-4 mr-1" /> Slots
//             </TabsTrigger>
//           </TabsList>
...
//             <Card>
//               <CardHeader>
//                 <CardTitle>{editingVenue ? "Edit Venue" : "Add Venue"}</CardTitle>
//               </CardHeader>
//               <CardContent>
//                 <form onSubmit={handleVenueSubmit} className="space-y-4">
...
//                         value={venueForm.block || ""}
//                         onChange={(e) =>
//                           setVenueForm({ ...venueForm, block: e.target.value })
//                         }
//                       />
//                     </div>
//                   </div>
...
//                         <TableCell>{v.block}</TableCell>
//                         <TableCell>{v.maxSeats}</TableCell>
//                         <TableCell>
//                           <Button
//                             variant="outline"
//                             size="sm"
...
//                             {slotDate ? format(slotDate, "PPP") : "Pick date"}
//                           </Button>
//                         </PopoverTrigger>
//                         <PopoverContent>
//                           <Calendar
...
import {
  ...
  updateSlot,
  deleteSlot,
} from "@/api/slotApi"; // <-- API functions imported

// Interfaces (kept from your old code)
interface Venue {
  id: number;
  name: string;
  block: string;
...
interface Slot {
  ...
  seatsLeft: number;
  createdBy: string;
  venue?: Venue; // This is important for the UI
  createdAt?: string;
  updatedAt?: string;
}

// Default form data (kept from your old code)
const defaultVenueData: Partial<Venue> = {
  name: "",
  block: "",
  maxSeats: 30,
...
const defaultSlotData: Partial<Slot> = {
  // date: new Date().toISOString().split('T')[0], // Handled by slotDate state
  startTime: "09:00",
  endTime: "10:00",
  venueId: 0,
  slotFor: "Internal",
...
const SlotBooking = () => {
  ...
  // Error boundary state (kept from old code)
  const [error, setError] = useState<string | null>(null);
  ...
  const loadSlots = useCallback(async () => {
    try {
      const data = await getSlots();
      // Assuming getSlots() returns slots with the 'venue' object nested
      // If not, you'd need to map them here, but your UI expects `slot.venue.name`
      setSlots(data);
    } catch (error) {
      toast({
        title: "Error loading slots",
  ...
  useEffect(() => {
    ...
    loadData();
  }, [loadVenues, loadSlots]); // Dependencies added

  // --- Venue Handlers (API-driven) ---
  const handleVenueSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!venueForm.name || !venueForm.block || !venueForm.maxSeats) {
      toast({
    ...
    try {
      ...
      await loadVenues(); // Refresh data from API
      setVenueForm(defaultVenueData);
      setEditingVenue(null);
    } catch (error) {
      toast({
        title: "Error",
        description:
          (error as any).message || "Failed to save venue",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleDeleteVenue = async (id: number) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: (error as any).message || "Failed to delete venue",
        variant: "destructive",
      });
    }
  };

  // --- Slot Handlers (API-driven) ---
  const handleSlotSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (
      !slotDate ||
    ...
    try {
      ...
      await loadSlots(); // Refresh data from API
      setSlotForm(defaultSlotData);
      setSlotDate(new Date());
      setEditingSlot(null);
    } catch (error) {
      toast({
        title: "Error",
        description: (error as any).message || "Failed to save slot",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleEditSlot = (slot: Slot) => {
    // We set the form to the full slot object
    setSlotForm(slot); 
    // The date needs to be a Date object for the calendar
    // setSlotDate(new Date(slot.date)); 
    setSlotDate(new Date(`${slot.date}T00:00:00`));
    setEditingSlot(slot);
  };
  ...
  const handleDeleteSlot = async (id: number) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: (error as any).message || "Failed to delete slot",
        variant: "destructive",
      });
    }
  };
  ...
  // Error boundary render (kept from old code)
  if (error) {
    return (
      <AdminLayout>
        <div className="p-6">
  ...
  // Page loading spinner (kept from old code)
  if (pageLoading) {
    return (
      <AdminLayout>
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto mb-4"></div>
            <p className="text-muted-foreground">
              Loading slot booking management...
            </p>
          </div>
        </div>
      </AdminLayout>
  ...
  // Main component render (kept from old code)
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
              ...
              <CardContent>
                ...
                {venues.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p>No venues found</p>
                    <p className="text-sm">
                      Create your first venue using the form above
                    </p>
                  </div>
                )}
              </CardContent>
                        ...
                        <SelectContent>
                          {venues.length === 0 ? (
                            ...
                            venues.map((venue) => (
                              ...
                              >
                                {venue.name} - {venue.block} ({venue.maxSeats}{" "}
                                seats)
                              </SelectItem>
                            ))
                          )}
                ...
                <Table>
                  ...
                  <TableBody>
                    {slots.map((slot) => (
                      <TableRow key={slot.id}>
                        <TableCell>
                          {/* Add time zone correction by parsing as UTC */}
                          {format(new Date(`${slot.date}T00:00:00`), "MMM dd, yyyy")}
                        </TableCell>
                        <TableCell>
