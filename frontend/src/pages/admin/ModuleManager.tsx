...
import {
  ...
  updateModule,
  deleteModule,
} from "@/api/moduleApi"; // Assuming api/moduleApi.ts exists

// --- Interfaces (from your older, detailed version) ---
interface Resource {
  // Define resource properties if needed, e.g., id, name, url
  id: number;
  name: string;
  url: string;
}
...
interface Module {
  ...
  description: string;
  duration: number;
  published: boolean; // This field is from your old interface
  mcqPassCriteria: number;
  codingPassCriteria: number;
  questionsToDisplay: number;
  randomizeQuestions: boolean;
...
// We still need this to load the *course* details (name, etc.)
const COURSES_STORAGE_KEY = 'courses';
...
const ModuleManager = () => {
  ...
  // State from your older, detailed version
  const [course, setCourse] = useState<any>(null); // For course name, etc.
  const [modules, setModules] = useState<Module[]>([]); // To be loaded from API
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [editingModule, setEditingModule] = useState<Module | null>(null);
  const [loading, setLoading] = useState(false);
  const [selectedModule, setSelectedModule] = useState<Module | null>(null);
  ...
  // Form state from your older, detailed version
  const [moduleForm, setModuleForm] = useState({
    title: '',
    description: '',
    duration: 60,
  ...
  // Load course details (from local) and modules (from API)
  useEffect(() => {
    if (courseId) {
      loadCourseDetails();
      fetchModules();
  ...
  /**
   * Loads the parent course's details from localStorage.
   * The UI needs this for the header and course name.
   */
  ...
  /**
   * Fetches the list of modules for this course from the API.
   * (Logic from your new version)
   */
  const fetchModules = async () => {
    try {
      setLoading(true);
      const data = await getModulesByCourse(Number(courseId));
      setModules(data);
    } catch (error) {
  ...
  /**
   * Handles create or update submission, calling the API.
   * (Merged logic)
   */
    ...
    try {
      if (editingModule) {
        ...
        await updateModule(editingModule.id, {
          ...moduleForm,
          // The 'published' boolean can be synced with the 'status'
          published: moduleForm.status === 'published', 
        });
        toast({ title: "Success", description: "Module updated successfully" });
      } else {
      ...
      fetchModules(); // Re-fetch modules from API
      resetModuleForm(); // Reset the form
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to save module",
  ...
  const handleDeleteModule = async (id: number) => {
    ...
    try {
      ...
      // If the deleted module was selected, unselect it
  ...
  // --- Start of JSX (from your older, detailed version) ---
      ...
      <AdminLayout>
        <div className="p-6">
          <div className="text-center py-12">
            ...
            <p>If this persists, the course may not exist.</p>
            <Button 
              onClick={() => navigate('/admin/courses')}
              className="mt-4"
            >
          ...
          <Card className="border-orange-200">
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                <Target className="h-8 w-8 text-orange-600" />
                <div>
                  {/* FIXED: Changed m.topics.length to m.resources.length */}
                  <p className="text-2xl font-bold">{modules.reduce((sum, m) => sum + (m.resources?.length || 0), 0)}</p>
