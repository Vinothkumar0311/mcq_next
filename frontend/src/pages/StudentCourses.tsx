...
import { useToast } from "@/hooks/use-toast";
import {
  BookOpen, Lock, Play, Target, Users, Calendar
} from "lucide-react";
import StudentLayout from "@/components/StudentLayout";
import { useNavigate } from "react-router-dom";
// 1. Import the courseApi
import { courseApi } from "@/api/courseApi"; // Adjust path as needed

// 2. Define API placeholders (These should be implemented in actual API files)
const enrollmentApi = {
  getEnrollmentsByStudent: async (studentId: string): Promise<any[]> => {
    // ⚠️ Placeholder: Replace with actual API call to get student's enrollments
    console.log(`Fetching enrollments for student: ${studentId}`);
    // Mock data structure: { courseId: number, enrolledAt: string, completedModules: number[] }
    return [
      { courseId: 1, enrolledAt: new Date().toISOString(), completedModules: [101] },
      { courseId: 2, enrolledAt: new Date().toISOString(), completedModules: [] },
    ];
  },
  enrollCourse: async (studentId: string, courseId: number) => {
    // ⚠️ Placeholder: Replace with actual API call to enroll
...
const moduleApi = {
  getModulesByCourse: async (courseId: number): Promise<Module[]> => {
    // ⚠️ Placeholder: Replace with actual API call to get published modules for a course
    console.log(`Fetching modules for course: ${courseId}`);
    // Mock data for Course ID 1
    if (courseId === 1) {
      return [
        { id: 101, courseId: 1, title: 'Module 1: Introduction', description: 'Intro to the topic', duration: 30, published: true, status: 'completed', score: 92 },
        { id: 102, courseId: 1, title: 'Module 2: Core Concepts', description: 'Deep dive into core concepts', duration: 45, published: true, status: 'available' },
        { id: 103, courseId: 1, title: 'Module 3: Advanced Topics', description: 'Advanced part of the course', duration: 60, published: true, status: 'locked' },
      ];
    }
    // Mock data for Course ID 2
    if (courseId === 2) {
      return [
        { id: 201, courseId: 2, title: 'Module A', description: 'First module', duration: 20, published: true, status: 'available' },
      ];
    }
    // Mock data for other courses
    return [];
  }
};
...
interface Course {
  ...
  image?: string;
  published: boolean;
  moduleCount: number; // You might need to infer this or fetch from API
  // Dynamic fields for student view
  progress: number;
  completedModules: number;
  totalModules: number;
  totalDuration: number;
...
const StudentCourses = () => {
  const { toast } = useToast();
  const navigate = useNavigate();
  const studentId = 'current_student'; // ⚠️ You should replace this with a real logged-in user ID
  ...
  // 3. Updated handler to use API
  const handleEnrollCourse = async (courseId: number) => {
    try {
      setLoading(true);
      await enrollmentApi.enrollCourse(studentId, courseId);
  ...
  // 4. Updated function to load modules from API
  const loadModulesForCourse = useCallback(async (courseId: number) => {
    try {
      const courseModules = await moduleApi.getModulesByCourse(courseId);
      
      // In a real app, you would fetch enrollment data specific to this course and student
      // and use it to determine the status. The mock data already contains the status.
      
      // Filter out any unpublished modules if the API doesn't handle it
      const publishedModules = courseModules.filter(m => m.published);
  ...
  // 5. Main function to load data from API
  // const loadCoursesAndProgress = useCallback(async () => {
  //   setLoading(true);
  //   try {
  //     // Fetch all courses
  //     const allCourses: Course[] = await courseApi.getCourses();
  //     const publishedCourses = allCourses.filter(course => course.published);

  //     // Fetch student enrollments
  //     const studentEnrollments = await enrollmentApi.getEnrollmentsByStudent(studentId);
  ...
  //         const totalModules = course.moduleCount || 5; // Use API count or mock
  ...
  //           enrolledAt: enrollment?.enrolledAt,
  //           // Assuming totalDuration is fetched with the course or calculated
  //           totalDuration: 120 // Mock
  //         });
  //       } else {
  ...
  //       const initialCourse = enrolled[0];
  //       setSelectedCourse(initialCourse);
  //       await loadModulesForCourse(initialCourse.id);
  //     } else if (selectedCourse) {
  //        // If a course was selected, re-load its modules and progress
  //        const updatedSelected = enrolled.find(c => c.id === selectedCourse.id) || null;
  //        if(updatedSelected) {
  //          setSelectedCourse(updatedSelected);
  //          await loadModulesForCourse(updatedSelected.id);
  //        }
  //     }
  ...
  //     setLoading(false);
  //   }
  // // }, [selectedCourse, studentId, loadModulesForCourse, toast]);
  // }, [studentId, loadModulesForCourse, toast]);
const loadCoursesAndProgress = useCallback(async () => {
  setLoading(true);
  try {
    ...
    publishedCourses.forEach(course => {
      if (enrolledIds.includes(course.id)) {
        ...
        enrolled.push({
          ...course,
          progress: (enrollment?.completedModules.length || 0) / totalModules * 100,
  ...
  // Handler for selecting an enrolled course
  ...
  const handleStartModule = (moduleId: number) => {
    // Check if slot is required or direct test access
    const hasActiveSlot = true; // Would check actual slot booking logic
    if (hasActiveSlot) {
      navigate(`/student/tests/${moduleId}`);
    } else {
      toast({
